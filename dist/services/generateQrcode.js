...
exports.makeIDCopyAndModifyPDF = exports.makeCopyAndModifyPDF = exports.generateQRCode = void 0;
...
async function appendQRCodeToPDF(qrCodeImagePath, name, expiry_date, ID, pdfPath) {
    try {
        // Read existing PDF
        ...
        page.drawText(name, {
            ...
            font: await pdfDoc.embedFont(pdf_lib_1.StandardFonts.Helvetica), // Change font here
        });
        ...
        page.drawText(ID, {
            ...
            size: 5, // Adjust font size as needed
            font: await pdfDoc.embedFont(pdf_lib_1.StandardFonts.Helvetica), // Change font here
            color: (0, pdf_lib_1.rgb)(1, 1, 1) // Red color
        });
...
//     }
// }
async function appendIDToPDF(qrCodeImagePath, name, expiry_date, ID, passportImageUrl, pdfPath) {
    try {
        // Read existing PDF
        const pdfBytes = fs.readFileSync(pdfPath);
        // Load PDF
        ...
        const response = await axios_1.default.get(passportImageUrl, { responseType: 'arraybuffer' });
        const passportImageBuffer = Buffer.from(response.data);
        const passportImageBytes = await (0, sharp_1.default)(passportImageBuffer)
            .resize(100, 100) // Adjust the size here for a larger image
            .composite([{
                input: Buffer.from('<svg><rect x="0" y="0" width="100" height="100" rx="50" ry="50"/></svg>', 'utf-8'),
                blend: 'dest-in'
            }])
            .png()
            .toBuffer();
...
async function makeCopyAndModifyPDF(originalFilePath, newFilePath, qrCodeData, qrCodePath, name, expiry_date, ID) {
    try {
...
exports.makeCopyAndModifyPDF = makeCopyAndModifyPDF;
async function makeIDCopyAndModifyPDF(originalFilePath, newFilePath, qrCodeData, qrCodePath, name, expiry_date, ID, passport) {
    try {
        // Read the contents of the original PDF file
        const pdfBytes = fs.readFileSync(originalFilePath);
        // Write the contents to a new file with a different name
...
exports.makeIDCopyAndModifyPDF = makeIDCopyAndModifyPDF;
// Example usage
