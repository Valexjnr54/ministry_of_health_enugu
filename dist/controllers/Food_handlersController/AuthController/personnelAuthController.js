...
async function loginPersonnel(req, res) {
    ...
    try {
        const user = await prisma.food_handlers.findFirst({
            where: {
                OR: [
                    { email: login_id },
        ...
        const token = jsonwebtoken_1.default.sign({ userId: user.id, email: user.enugu_handler_id }, config_1.Config.Jwt_secret);
...
async function profile(request, response) {
    ...
    try {
        ...
        const existinguser = await prisma.food_handlers.findUnique({ where: { id: user_id } });
        if (!existinguser) {
        ...
        const lga_id = parseInt(user.lga);
        const lga = await prisma.local_government.findUnique({ where: { id: lga_id } });
        return response.status(200).json({ message: 'user profile', user, lga });
    }
    catch (error) {
        console.error(error);
