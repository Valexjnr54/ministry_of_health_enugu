...
async function initializeFoodRenewalPaymentHandler(request, response) {
    try {
        ...
        const validationRules = [
            (0, express_validator_1.body)("enugu_food_facility_id")
                .notEmpty()
        ...
        const callback_url = config_1.Config.credoVerifyFoodFacilityRenewalCallback;
        if (!callback_url) {
            return response
                .status(400)
                .json({ message: "Callback Can't be undefined" });
        }
        const paymentInfo = await (0, credo_1.initializeFacilityPayment)(facility.enugu_food_facility_id, facility.owner_phone_number, payamount, email, facility_type, "Renewal Payment of Food Handler Facility", callback_url);
