...
async function createFacility(request, response) {
    ...
    try {
        const validationRules = [
            ...
            (0, express_validator_1.body)('personnel_incharge').notEmpty().withMessage('Personnel In Charge is required'),
            (0, express_validator_1.body)('personnel_incharge_name').notEmpty().withMessage('Name of Personnel In Charge is required'),
            (0, express_validator_1.body)('personnel_incharge_practice_id').notEmpty().withMessage('Practice ID of Personnel In Charge is required'),
        ];
        ...
        if (personnel_incharge === "Mid_wife") {
            const validationRules = [
                (0, express_validator_1.body)('supervising_doctor_name').notEmpty().withMessage('Supervising Doctor Name is required'),
                (0, express_validator_1.body)('supervising_doctor_practice_id').notEmpty().withMessage('Supervising Doctor Practice ID is required'),
            ];
            // Apply validation rules to the request
            await Promise.all(validationRules.map(rule => rule.run(request)));
            const errors = (0, express_validator_1.validationResult)(request);
        ...
        if (facility_type === "Specialist_hospital" || facility_type === "Specialist_clinic") {
            if (number_of_bed > 10) {
                amount = 300000;
            }
        ...
        else if (facility_type === "Grade_1_medical_lab" || facility_type === "Specialist_eye_hospital" || facility_type === "Mortuary") {
            amount = 300000;
        }
        else if (facility_type === "Grade_2_medical_lab" || facility_type === "Eye_clinic" || facility_type === "Radiotherapy_center" || facility_type === "Xray_center" || facility_type === "Dental_clinic") {
            amount = 150000;
        }
        else if (facility_type === "Grade_3_medical_lab" || facility_type === "Grade_4_medical_lab") {
            amount = 60000;
        }
        else if (facility_type === "Dental_center" || facility_type === "Outpatient_medical_clinic" || facility_type === "Eye_center") {
            amount = 75000;
        }
        else if (facility_type === "Physiotherapy_center" || facility_type === "Ultrasound") {
            amount = 120000;
        }
        else {
        ...
        if (!request.files || !('site_plan' in request.files) || !('owner_passport' in request.files) || !('cac_image' in request.files) || !('building_plan' in request.files) || !('tax_clearance_certificate' in request.files)) {
            return response.status(400).json({ message: 'Please provide all required images.' });
        }
        let owner_passport;
        if (request.files['owner_passport'][0].mimetype == 'image/png' || request.files['owner_passport'][0].mimetype == 'image/jpeg' || request.files['owner_passport'][0].mimetype == 'image/jpg' || request.files['owner_passport'][0].mimetype == 'image/gif') {
        ...
        let cac_image;
        if (request.files['cac_image'][0].mimetype == 'image/png' || request.files['cac_image'][0].mimetype == 'image/jpeg' || request.files['cac_image'][0].mimetype == 'image/jpg' || request.files['cac_image'][0].mimetype == 'image/gif') {
            // Upload Site Plan image to Cloudinary
        ...
        let site_plan;
        if (request.files['site_plan'][0].mimetype == 'image/png' || request.files['site_plan'][0].mimetype == 'image/jpeg' || request.files['site_plan'][0].mimetype == 'image/jpg' || request.files['site_plan'][0].mimetype == 'image/gif') {
            // Upload Site Plan image to Cloudinary
        ...
        let building_plan;
        if (request.files['building_plan'][0].mimetype == 'image/png' || request.files['building_plan'][0].mimetype == 'image/jpeg' || request.files['building_plan'][0].mimetype == 'image/jpg' || request.files['building_plan'][0].mimetype == 'image/gif') {
        ...
        if (request.files['tax_clearance_certificate'][0].mimetype == 'image/png' || request.files['tax_clearance_certificate'][0].mimetype == 'image/jpeg' || request.files['tax_clearance_certificate'][0].mimetype == 'image/jpg' || request.files['tax_clearance_certificate'][0].mimetype == 'image/gif') {
        ...
        const callback_url = config_1.Config.paystackFacilityCompleteRegistrationCallback;
        if (!callback_url) {
        ...
        const paymentInfo = await (0, paystack_1.initializeCompletePayment)(facility_name, facility.id, owner_phone_number, payamount, owner_email, facility_type, type_of_payment, callback_url);
...
async function saveAndExit(request, response) {
    ...
    try {
        ...
        return response.status(201).json({ message: "Details Saved, Login to Complete Registration", user: facility, lga: lga_detail });
    }
    catch (error) {
        console.log(error);
        return response.status(500).json({ message: 'Internal Server Error' });
...
async function verifyCompleteRegistrationPaymentHandler(request, response) {
    try {
        ...
        const facility_id = parseInt(paymentDetails.metadata.facility_id);
        ...
        if (facility_complete) {
            const fetch_facility = await prisma.facility.findUnique({ where: { id: facility_id } });
            return response.status(200).json({ message: 'Payment was Successful', facility: facility_complete });
        }
        ...
        return response.status(200).json({ message: 'Payment was Successful', user: facility, lga });
    }
    catch (error) {
        console.error(error);
...
async function initializeRenewalPaymentHandler(request, response) {
    try {
        ...
        const validationRules = [
            (0, express_validator_1.body)('enugu_facility_id').notEmpty().withMessage('Enugu State Facility Id is required'),
        ];
        // Apply validation rules to the request
        await Promise.all(validationRules.map(rule => rule.run(request)));
        const errors = (0, express_validator_1.validationResult)(request);
        ...
        if (facility.status !== "Approved") {
            return response.status(400).json({ message: 'Your Facility has not been approved yet' });
        }
        ...
        const payamount = amount * 100;
        const callback_url = config_1.Config.paystackVerifyFacilityRenewalCallback;
        if (!callback_url) {
            return response.status(400).json({ message: "Callback Can't be undefined" });
        }
        const paymentInfo = await (0, paystack_1.initializeFacilityPayment)(facility.enugu_facility_id, facility.owner_phone_number, payamount, email, facility.facility_type, type_of_payment, callback_url);
...
async function verifyRenewalPaymentHandler(request, response) {
    try {
        ...
        if (check_reference) {
            ...
            return response.status(200).json({ message: 'Renewal was Successful', user: check_facility, payment: check_reference });
        }
        ...
        return response.status(200).json({ message: 'Renewal was Successful', user: facility, payment, lga });
    }
    catch (error) {
        console.log(error);
    }
...
async function inspectionResult(request, response) {
    ...
    try {
        ...
        const result = await prisma.inspection_review.findFirst({ where: { facility_id: id } });
        if (!result) {
            return response.status(404).json({ message: "Inspection result does not exist" });
        }
        return response.status(200).json({ message: "Inspection Result fetched successfully", result });
    }
    catch (error) {
        console.log(error);
        return response.status(500).json({ message: 'Internal Server Error' });
