...
async function verifyPersonnel(request, response) {
    const enugu_practice_id = request.query.enugu_practice_id;
    try {
        if (enugu_practice_id !== undefined) {
            ...
            const callback_url = config_1.Config.paystackPersonnelVerificationCallback;
            if (!callback_url) {
            ...
            const paymentInfo = await (0, paystack_1.initializePayment)(checkuser?.fullname, checkuser?.phone_number, amount, checkuser?.email, checkuser?.type, type_of_payment, callback_url);
...
async function verifyFoodPersonnel(request, response) {
    const enugu_handler_id = request.query.enugu_handler_id;
    try {
        if (enugu_handler_id !== undefined) {
            ...
            const amount = 2000 * 100;
            const callback_url = config_1.Config.paystackFoodPersonnelVerificationCallback;
            if (!callback_url) {
                return response.status(400).json({ message: "Callback Can't be undefined" });
            }
            const type_of_payment = "Payment for Food Handler Personnel Verification";
            const type = "Food Handler";
            const paymentInfo = await (0, paystack_1.initializePayment)(checkuser?.fullname, checkuser?.phone_number, amount, checkuser?.email, type, type_of_payment, callback_url);
            return response.status(200).json({ message: `${checkuser?.fullname} has been fetched Successfully`, user: checkuser, paymentInfo });
        }
        else {
            return response.status(400).json({ message: 'enugu_practice_id is required' });
...
async function verifyFacility(request, response) {
    const enugu_facility_id = request.query.enugu_facility_id;
    try {
        if (enugu_facility_id !== undefined) {
            ...
            const callback_url = config_1.Config.paystackFacilityVerificationCallback;
            if (!callback_url) {
                return response.status(400).json({ message: "Callback Can't be undefined" });
            }
            const paymentInfo = await (0, paystack_1.initializePayment)(enugu_facility_id, checkfacility?.owner_phone_number, amount, checkfacility?.owner_email, checkfacility?.facility_type, type_of_payment, callback_url);
...
async function verifyFoodFacility(request, response) {
    const enugu_food_facility_id = request.query.enugu_food_facility_id;
    try {
        if (enugu_food_facility_id !== undefined) {
            ...
            const amount = 5000 * 100;
            const callback_url = config_1.Config.paystackFoodFacilityVerificationCallback;
            if (!callback_url) {
                return response.status(400).json({ message: "Callback Can't be undefined" });
            }
            const paymentInfo = await (0, paystack_1.initializePayment)(enugu_food_facility_id, checkfacility?.owner_phone_number, amount, checkfacility?.owner_email, checkfacility?.facility_name, type_of_payment, callback_url);
            return response.status(200).json({ message: `${checkfacility?.facility_name} has been fetched Successfully`, user: checkfacility, paymentInfo });
        }
        else {
            return response.status(400).json({ message: 'enugu_facility_id is required' });
