...
import {
  sendFoodPaymentEmail,
  sendFoodReceiptEmail,
  sendRenewalEmail,
  sendRenewalFoodHandlerEmail,
} from "../../../utils/emailSender";
import { personnel_price_list_service } from "../../../services/calculatePrice";

const prisma = new PrismaClient();

export async function initializeFoodPaymentHandler(
  request: Request,
  response: Response
) {
  try {
    ...
    const callback_url = Config.credoFoodPersonnelCallback;
...
export async function verifyFoodPaymentHandler(
  request: Request,
  response: Response
) {
  try {
...
export async function initializeFoodRenewalPaymentHandler(
  ...
  try {
    ...
    const type = "Food handler";
    const payamount = amount * 100;
    const callback_url = Config.credoVerifyPersonnelRenewalCallback;
    if (!callback_url) {
      return response
        .status(400)
        .json({ message: "Callback Can't be undefined" });
