...
export async function add_price_list(request: Request, response: Response) {
    const { type, application_fee, application_charge, certificate_charge, renewal_fee, renewal_charge } = request.body;
  ...
  try {
    ...
    const addPriceList = await prisma.personnel_price_list.create({
        data:{
            ...
            application_fee,
            application_charge,
            enugu_ID_certificate:certificate_charge,
            renewal_charge,
            renewal_fee
        }
    })
...
export async function add_facility_price_list(request: Request, response: Response) {
  const { type, number_of_beds, first_time_reg, first_time_charge, application_fee, application_charge, renewal_fee, renewal_charge } = request.body;
  const admin_id = request.user.adminId;
...
export async function add_facility_constant_price_list(request: Request, response: Response) {
...
try {
  const validationRules = [
      body('name').notEmpty().withMessage('Title of Fee is required'),
  ...
  const addPriceList = await prisma.facility_constant_price_list.create({
      data:{
          name,
          amount
      }
...
export async function view_facility_constant_price_lists(request: Request, response: Response) {
  const userId = request.user.adminId;
const tokencheck = request.headers.authorization?.split(' ')[1];
try {
    // Check if the email is already registered
    ...
    // Create a new user in the database
    const priceList = await prisma.facility_constant_price_list.findMany();
...
export async function view_facility_constant_single_price_list(request: Request, response: Response) {
  const admin_id = request.user.adminId;
  const priceListId = parseInt(request.query.id as string)
...
try {
  ...
  const list = await prisma.facility_constant_price_list.findUnique({ where: { id: priceListId }})
  return response.status(200).json({ message: `Price List`, data:list });
} catch (error) {
  return response.status(500).json({ message: error})
}
...
export async function update_facility_constant_price_list(request: Request, response: Response) {
  const { name, amount } = request.body;
  const admin_id = request.user.adminId;
  const priceListId = parseInt(request.query.id as string)
...
try {
  ...
  const updateList = await prisma.facility_constant_price_list.update({
      where:{
          id: priceListId
      },
      data:{
  ...
  return response.status(201).json({  message: `Price List has been created`, data: updateList });
} catch (error) {
  return response.status(500).json({ message: error})
}
}

export async function delete_facility_constant_price_list(request: Request, response: Response) {
  const admin_id = request.user.adminId;
  const priceListId = parseInt(request.query.id as string)
...
try {
  ...
  const list = await prisma.facility_constant_price_list.delete({ where: { id: priceListId }})
  return response.status(204).json({ message: `Price List has been deleted successfully` });
} catch (error) {
  return response.status(500).json({ message: error})
}
}