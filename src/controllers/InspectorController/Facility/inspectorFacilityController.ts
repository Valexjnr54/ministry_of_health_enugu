...
import { sendApprovedInspectionMail, sendRejectedInspectionMail } from '../../../utils/emailSender';

const prisma = new PrismaClient();

export async function facilities(request: Request, response: Response) {
...
export async function facilitiesByLGA(request: Request, response: Response) {
    ...
    try {
        ...
        const int_lga = parseInt(lga as string)
...
export async function facilitiesByInspectorID(request: Request, response: Response) {
    const inspector_id = request.user.inspectorId;
...
export async function facility(request: Request, response: Response) {
    const id = parseInt(request.query.facility_id as string)
    const inspector_id = request.user.inspectorId;
    ...
    try {
        const validationRules = [
            body('facility_id').notEmpty().withMessage('Facility ID is required'),
          ];
...
export async function inspectionItems(request: Request, response: Response) {
    const id = parseInt(request.query.facility_id as string)
    const inspector_id = request.user.inspectorId;
...
export async function inspectionReview(request: Request, response: Response) {
    const inspector_id = request.user.inspectorId;
    const {
        facility_id,
        cac,
    ...
    try {
        const validationRules = [
            body('facility_id').notEmpty().withMessage('Facility ID is required'),
            body('cac').isBoolean().withMessage('CAC field must be a boolean value'),
            body('owner_nin').isBoolean().withMessage('Owner NIN field must be a boolean value'),
            body('services_offered').isBoolean().withMessage('Services offered field must be a boolean value'),
            body('number_of_bed').isBoolean().withMessage('Number of bed field must be a boolean value'),
            body('source_of_water').isBoolean().withMessage('Source of water field must be a boolean value'),
            body('method_of_disposal').isBoolean().withMessage('Method of disposal field must be a boolean value'),
            body('site_plan').isBoolean().withMessage('Site plan field must be a boolean value'),
            body('building_plan').isBoolean().withMessage('Building plan field must be a boolean value'),
            // body('compliance_certificate').isBoolean().withMessage('Compliance certificate field must be a boolean value'),
            body('tax_clearance_certificate').isBoolean().withMessage('Tax clearance certificate field must be a boolean value'),
            body('facility_personnel_count').isBoolean().withMessage('Facility Personnel Count field must be a boolean value'),
            body('facility_personnel').isBoolean().withMessage('Facility Personnel field must be a boolean value'),
        ...
        if (update_inspection.status == "Approved") {
            await sendApprovedInspectionMail(facility.owner_email,facility,facility.facility_name,facility.enugu_facility_id, facility.expiry_date, update_inspection)
        } else {
            await sendRejectedInspectionMail(facility.owner_email,facility,facility.facility_name,facility.enugu_facility_id, facility.expiry_date, update_inspection)
        }
        const lga_id = parseInt(facility.lga as string)
        const lga = await prisma.local_government.findUnique({ where:{id: lga_id}})
        return response.status(200).json({ message: "Review has been submitted", review: update_inspection, facility, lga})
...
export async function monitor(request: Request, response: Response) {
