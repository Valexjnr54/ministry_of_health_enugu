...
export async function loginFacility(req: Request, res: Response) {
  const { login_id, password } = req.body;
  try {
    const facility = await prisma.food_facility.findFirst({
      where: {
        OR: [
          { enugu_food_facility_id: login_id }
        ]
      }
    });
    ...
    const token = jwt.sign({ facilityId: facility.id, email: facility.enugu_food_facility_id }, Config.Jwt_secret);
...
export async function logoutfacility(req: Request, res: Response) {
  try {
...
export async function profile(request: Request, response: Response) {
  ...
  try {
      ...
      const existingFacility = await prisma.food_facility.findUnique({ where: { id:facility_id } });
      if (!existingFacility) {
