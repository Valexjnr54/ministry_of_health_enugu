...
export async function createFacility(request: Request, response: Response) {
  ...
  try {
    ...
    if (
      !request.files ||
      !("owner_nin_image" in request.files) ||
    ...
    const owner_nin_image = await uploadImage(
      request.files["owner_nin_image"][0].path,
      "enugu_ministry_of_health/images/owner_nin_images"
    );
...
export async function initializeFoodRenewalPaymentHandler(
  request: Request,
  response: Response
) {
  try {
    ...
    const validationRules = [
      body("enugu_food_facility_id")
        .notEmpty()
        .withMessage("Enugu State Food Vendor Facility Id is required"),
    ];
    ...
    const callback_url = Config.credoVerifyFoodFacilityRenewalCallback;
    if (!callback_url) {
      return response
        .status(400)
    ...
    const paymentInfo = await initializeFacilityPayment(
      facility.enugu_food_facility_id,
      facility.owner_phone_number,
      payamount,
      email,
      facility_type,
      "Renewal Payment of Food Handler Facility",
      callback_url
    );
...
export async function verifyFoodRenewalPaymentHandler(
  request: Request,
  response: Response
) {
  try {
    ...
    return response
      .status(200)
      .json({ message: "Renewal was Successful", facility, payment });
  } catch (error) {
    console.log(error);
  }
}
