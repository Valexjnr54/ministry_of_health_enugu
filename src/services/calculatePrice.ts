...
export async function facility_price_list_service(type: any, number_of_beds: string | null) {
    try {
        ...
        if (!verification || !inspection_charge || !inspection_fee || !certificate) {
            return ({ message: `One or more constant fees are invalid` });
        }
        ...
        const total_application_fee = charge_fee + application_fee + verification_fee;
        const total_first_time_fee = first_time_reg + first_time_charge_fee + inspection_charge_fee + inspection_fee_amount + certificate_fee;
        const total_renewal_fee = renewal_fee + renewal_charge_fee;
